  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to create kamar')
      }

      // Redirect to kamar list
      router.push('/dashboard/kamar')
    } catch (error: any) {
      console.error('Submit error:', error)
      setErrors({ submit: error.message || 'Gagal menambahkan kamar' })
    } finally {
      setLoading(false)
    }
  }
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-800">Tambah Kamar Baru</h1>
